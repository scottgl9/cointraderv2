...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Awesome Oscillator indicator')
    ...
    for candle in reversed(candles):
        ...
        result = ao.update(kline)
        if result is None:
            result = np.nan
        ao_values.append(result)
    ...
    ao_plot = mpf.make_addplot(ao_values, panel=1, color='blue', width=1.5)

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Awesome Oscillator',
        ylabel='Price',
        addplot=[ao_plot],
        panel_ratios=(3, 1),
    )
