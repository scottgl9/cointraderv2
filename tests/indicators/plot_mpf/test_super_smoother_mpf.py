...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Super Smoother indicator')
    ...
    super_smoother = SuperSmoother('SuperSmoother', 10)
    ...
    for candle in reversed(candles):
        ...
        result = super_smoother.update(kline)
        if super_smoother.ready():
            super_smoother_values.append(result)
        else:
            super_smoother_values.append(np.nan)
    ...
    super_smoother_plot = mpf.make_addplot(super_smoother_values, panel=0, color='blue', width=1.5)

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Super Smoother',
        ylabel='Price',
        addplot=[super_smoother_plot],
    )
