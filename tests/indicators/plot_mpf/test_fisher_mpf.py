...
from cointrader.indicators.FisherTransform import FisherTransform
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Fisher Transform indicator')
    ...
    fisher = FisherTransform(length=9)
    fisher_values = []
    signal_values = []
    ...
    for candle in reversed(candles):
        ...
        result = fisher.update(kline)
        if fisher.ready() and result is not None:
            fisher_values.append(result['fisher'])
    ...
    fisher_plots = [
        mpf.make_addplot(fisher_values, panel=1, color='blue', width=1.5, ylabel='Fisher'),
        mpf.make_addplot(signal_values, panel=1, color='red', width=1.5),
    ]
    ...
    mpf.plot(
        ...
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Fisher Transform',
        ylabel='Price',
        addplot=fisher_plots,
    )
