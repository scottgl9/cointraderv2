...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot PPO indicator')
    ...
    ppo_indicator = PPO(short_period=12, long_period=26, signal_period=9)
    ppo_values = []
    signal_values = []
    histogram_values = []
    ...
    for candle in reversed(candles):
        ...
        result = ppo_indicator.update(kline)
        if ppo_indicator.ready():
            ppo_values.append(result['ppo'])
            signal_values.append(result['signal'])
            histogram_values.append(result['histogram'])
        else:
            ppo_values.append(np.nan)
    ...
    ppo_plot = mpf.make_addplot(ppo_values, panel=1, color='blue', width=1.5, ylabel='PPO')
    signal_plot = mpf.make_addplot(signal_values, panel=1, color='red', width=1.5)
    histogram_plot = mpf.make_addplot(histogram_values, panel=1, type='bar', color='green', width=0.5)

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with PPO',
        ylabel='Price',
        addplot=[ppo_plot, signal_plot, histogram_plot],
    )
