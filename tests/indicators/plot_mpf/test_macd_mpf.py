...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot MACD indicator')
    ...
    macd_indicator = MACD(short_period=12, long_period=26, signal_period=9)
    macd_values = []
    signal_values = []
    histogram_values = []
    ...
    for candle in reversed(candles):
        ...
        result = macd_indicator.update(kline)
        if macd_indicator.ready():
            macd_values.append(result['macd'])
    ...
    macd_plot = mpf.make_addplot(macd_values, panel=1, color='blue', width=1.5, ylabel='MACD')
    ...
    mpf.plot(
        ...
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with MACD',
        ylabel='Price',
