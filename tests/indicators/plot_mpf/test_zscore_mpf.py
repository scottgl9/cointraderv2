...
from cointrader.indicators.ZScore import ZScore
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot ZScore indicator')
    ...
    for candle in reversed(candles):
        ...
        result = zscore.update(kline)
        if zscore.ready():
            zscore_values.append(result)
        else:
    ...
    zscore_plot = mpf.make_addplot(zscore_values, panel=1, color='blue', width=1.5, ylabel='ZScore')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with ZScore',
        ylabel='Price',
        volume=True,
        volume_panel=0,
        addplot=[zscore_plot],
