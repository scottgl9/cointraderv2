...
from cointrader.indicators.EOM import EaseOfMovement as EOM
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot DPO indicator')
    ...
    dpo = DetrendedPriceOscillator(period=14)
    dpo_values = []
    ...
    for candle in reversed(candles):
        ...
        result = dpo.update(kline)
        if dpo.ready():
            dpo_values.append(result)
        else:
            dpo_values.append(np.nan)
    ...
    dpo_plot = mpf.make_addplot(dpo_values, panel=1, color='blue', width=1.5, ylabel='DPO')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with DPO',
        ylabel='Price',
        addplot=[dpo_plot],
    )
