...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot KST indicator')
    ...
    for candle in reversed(candles):
        ...
        result = kst.update(kline)
        if not kst.ready() or result is None:
            kst_values.append(np.nan)
            signal_values.append(np.nan)
        else:
            kst_values.append(result["kst"])
    ...
    kst_plot = mpf.make_addplot(kst_values, panel=1, color='blue', width=1.5, ylabel='KST')
    signal_plot = mpf.make_addplot(signal_values, panel=1, color='red', width=1.5, secondary_y=True, ylabel='Signal')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with KST',
        ylabel='Price',
        addplot=[kst_plot, signal_plot],
        panel_ratios=(3, 1),
    )
