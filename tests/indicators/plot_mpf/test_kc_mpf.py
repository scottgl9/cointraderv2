...
from cointrader.indicators.KeltnerChannels import KeltnerChannels
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Keltner Channels indicator')
    ...
    kc = KeltnerChannels(period=20, multiplier=2)
    kc_upper_values = []
    kc_middle_values = []
    kc_lower_values = []
    ...
    for candle in reversed(candles):
        ...
        result = kc.update(kline)
        if not kc.ready() or result is None:
            result = {}
            result['upper'] = np.nan
        ...
        lower = result['lower']
        kc_upper_values.append(upper)
        kc_middle_values.append(middle)
        kc_lower_values.append(lower)
    ...
    kc_upper_plot = mpf.make_addplot(kc_upper_values, panel=0, color='blue', width=1.5)
    kc_middle_plot = mpf.make_addplot(kc_middle_values, panel=0, color='red', width=1.5)
    kc_lower_plot = mpf.make_addplot(kc_lower_values, panel=0, color='blue', width=1.5)

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Keltner Channels',
        ylabel='Price',
        addplot=[kc_upper_plot, kc_middle_plot, kc_lower_plot],
    )
