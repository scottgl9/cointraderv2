...
from cointrader.indicators.PriceChannel import PriceChannel
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Price Channel indicator')
    ...
    for candle in reversed(candles):
        ...
        result = price_channel.update(kline)
        if price_channel.ready():
            high_values.append(result['high'])
            low_values.append(result['low'])
            middle_values.append(result['middle'])
            print(f"High: {result['high']}, Low: {result['low']}, Middle: {result['middle']}")
        else:
    ...
    high_plot = mpf.make_addplot(high_values, panel=0, color='blue', width=1.5)
    low_plot = mpf.make_addplot(low_values, panel=0, color='red', width=1.5)
    middle_plot = mpf.make_addplot(middle_values, panel=0, color='green', width=1.5)

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Price Channel',
        ylabel='Price',
        addplot=[high_plot, low_plot, middle_plot],
    )
