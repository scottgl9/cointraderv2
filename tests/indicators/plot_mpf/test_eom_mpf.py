...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot EOM indicator')
    ...
    for candle in reversed(candles):
        ...
        result = eom.update(kline)
        if eom.ready():
            eom_values.append(result)
        else:
            eom_values.append(np.nan)
    ...
    eom_plot = mpf.make_addplot(eom_values, panel=1, color='blue', width=1.5, ylabel='EOM')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with EOM',
        ylabel='Price',
        addplot=[eom_plot],
    )
