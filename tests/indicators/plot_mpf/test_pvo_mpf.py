...
from cointrader.indicators.PVO import PVO
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot PVO indicator')
    ...
    for candle in reversed(candles):
        ...
        result = pvo_indicator.update(kline)
        if pvo_indicator.ready():
            pvo_values.append(result['pvo'])
            signal_values.append(result['signal'])
            histogram_values.append(result['histogram'])
        else:
            pvo_values.append(np.nan)
    ...
    pvo_plot = mpf.make_addplot(pvo_values, panel=1, color='blue', width=1.5, ylabel='PVO')
    ...
    mpf.plot(
        ...
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with PVO',
        ylabel='Price',
        addplot=[pvo_plot, signal_plot, histogram_plot],
    )
