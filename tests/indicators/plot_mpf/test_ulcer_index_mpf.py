...
from cointrader.indicators.UlcerIndex import UlcerIndex
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Ulcer Index indicator')
    ...
    for candle in reversed(candles):
        ...
        result = ulcer_index.update(kline)
        if ulcer_index.ready():
            ulcer_values.append(result)
        else:
    ...
    ulcer_plot = mpf.make_addplot(ulcer_values, panel=1, color='blue', width=1.5, ylabel='Ulcer Index')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Ulcer Index',
        ylabel='Price',
        addplot=[ulcer_plot],
    )
