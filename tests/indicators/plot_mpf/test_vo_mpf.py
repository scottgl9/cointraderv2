...
from cointrader.indicators.VO import VolumeOscillator as VO
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Volume Oscillator indicator')
    ...
    vo = VO(short_period=5, long_period=10)
    vo_values = []
    ...
    for candle in reversed(candles):
        ...
        result = vo.update(kline)
        if vo.ready():
            vo_values.append(result)
        else:
            vo_values.append(np.nan)
    ...
    vo_plot = mpf.make_addplot(vo_values, panel=1, color='blue', width=1.5, ylabel='VO')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Volume Oscillator',
        ylabel='Price',
        addplot=[vo_plot],
    )
