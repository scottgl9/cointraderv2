...
from cointrader.indicators.CoppockCurve import CoppockCurve
from cointrader.common.Kline import Kline

CLIENT_NAME = "cbadv"

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Coppock Curve indicator')
    ...
    for candle in reversed(candles):
        ...
        result = coppock_curve.update(kline)
        if coppock_curve.ready():
            coppock_values.append(result)
        else:
            coppock_values.append(np.nan)
    ...
    coppock_plot = mpf.make_addplot(coppock_values, panel=1, color='purple', width=1.5, ylabel='Coppock Curve')

    mpf.plot(
        df,
        type='candle',
        style='charles',
        title=f'{ticker} {granularity_name} chart with Coppock Curve',
        ylabel='Price',
        addplot=[coppock_plot],
    )
